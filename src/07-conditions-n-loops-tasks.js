...
function isCreditCardNumber(ccn) {
  ...
  const cardDigits = ccn.toString().split('');
  for (let i = cardDigits.length - 2; i >= 0; i -= 2) {
    cardDigits[i] = (cardDigits[i] * 2).toString();
    if (cardDigits[i] >= 10) {
      cardDigits[i] = (parseInt(cardDigits[i], 10) - 9).toString();
    }
  }
  const sum = cardDigits.reduce((acc, cur) => acc + parseInt(cur, 10), 0);
  return sum % 10 === 0;
}
